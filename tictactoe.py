...
def print_board(sign_lst):
	...
	(lambda: os.system('clear'))() #clear console on linux 'cls' for windows systems
	for i in range(1,10,3):
		if i < 6:
			divider = '-'*11 #prints 11 '-'
		else:
			divider = ''
		print(f'{sign_lst[-i-2]:>2} | {sign_lst[-i-1]:^1} | {sign_lst[-i]:2} \n{divider}')
...
def player_input():
	...
	while not (player_1 == "O" or player_1 == "X"):
		player_1 = input("Enter your mark 'O' or 'X': ")
	else:
		if player_1 == "X":
			player_2 = "O"
...
def win_check(board, mark, player):
	...
	'''
	Won = (board[1:4:] == [mark]*3) or (board[4:7:]== [mark]*3) or (board[7::]== [mark]*3) or (board[1::3]== [mark]*3) or (board[2::3]== [mark]*3) or (board[3::3]== [mark]*3) or (board[1::4]== [mark]*3) or (board[3:8:2]== [mark]*3)
	if Won:
...
def game_start():
	...
	while not full_board_check(board):
		...
		place_marker(board, marks[first_player-1], position) #checks if the position is available, sets position
		print_board(board)
		if win_check(board, marks[first_player-1], first_player) or full_board_check(board):
